...
function CustomerDetails() {
  ...
  const fetchItems = useCallback(async () => {
    try {
      const itemsRes = await axios.get(`http://localhost:5000/items/${id}`);
  ...
  const fetchBills = useCallback(async () => {
    try {
      const billsRes = await axios.get(`http://localhost:5000/bills/customer/${id}/bills`);
  ...
  useEffect(() => {
    const fetchCustomerDetails = async () => {
      setLoading(true);
      try {
        const customerRes = await axios.get(`http://localhost:5000/customers/${id}`);
  ...
  const handleUpdateItem = (itemId, updatedItem) => {
      axios.post(`http://localhost:5000/items/update/${itemId}`, updatedItem)
        .then(res => {
            console.log(res.data);
            fetchItems(); // Refetch
        })
  ...
  const handleSaveExcelData = async () => {
    try {
      await axios.post(`http://localhost:5000/items/bulk-update/${customer._id}`, { items: excelData });
