...
function Billing() {
  ...
  useEffect(() => {
    if (selectedCustomer) {
      axios.get(`${process.env.REACT_APP_API_URL}/bills/customer/${selectedCustomer}/items`)
        .then(response => {
  ...
  const checkInventory = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/inventory/check-availability`, {
  ...
  const handleSubmitBill = async () => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/bills/add`, {
