...
function Inventory() {
  ...
  const handleSubmit = (e) => {
    ...
    if (editingItem) {
      ...
      axios.post(`http://localhost:5000/inventory/update/${editingItem._id}`, formData)
        .then(() => {
      ...
      // Add new item
      axios.post('http://localhost:5000/inventory/add', formData)
        .then(() => {
  ...
  const handleQuantityChange = (id, action, amount = 1) => {
    axios.post(`http://localhost:5000/inventory/update-quantity/${id}`, { action, amount })
      .then(() => {
        fetchInventory();
      })
      .catch((error) => {
